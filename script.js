 ...
 const winCombos = [[0, 1, 2], [3, 4, 5], [6, 7, 8], [0, 4, 8], [2, 4, 6], [2, 5, 8], [1, 4, 7], [0, 3, 6]];
...
const gameOver = gameWon => {
    for (let index of winCombos[gameWon.index]) {
        ...
        document.getElementById(index).style.backgroundColor = '#067667d6';
    }
    ...
    declareWinner(gameWon.player === user ? "hooray! You Won" : "You Loss! computer Win");
}
...
const emptySquares = () => {
 ...
 const checkTie = () => {
    if (emptySquares().length === 0) {
        ...
        for (let cell of cells) {
            cell.style.backgroundColor = "#067667d6";
            cell.removeEventListener('click', handleClick, false);
        }

        declareWinner("Opps! Game Tie");
        return true;
    }
